...
- lbm：除脂體重 LBM（kg）
  ...
  - 常見約 60~100 kg
- bmr（基礎代謝率）：右側「研究參數」區塊，單位 kcal，常見 1200~3500；勿與其他熱量欄混淆
- tdee（建議的熱量攝取）：同區最後一行「建議的熱量攝取／Recommended Calorie Intake」，通常明顯高於 BMR，常見 2000~6000
- body_water：體水分（kg）
- visceral_fat（內臟脂肪級別）：右側「內臟脂肪級別」那一行的數字（範圍 1~30），勿讀圖表刻度軸上的刻度值
- waist_hip_ratio：腰臀比（小數，例如 1.01）
- test_date：檢測日期（YYYY-MM-DD）

【版面與生理合理性】
- 若讀出 bmr > tdee，代表欄位可能對調，請重新對照報告後再填
- summary 中勿將 BMR 與「建議熱量攝取」標籤互換
...
3. bmr 與 tdee 皆存在時，tdee 必須 > bmr × 1.05；若不成立，重新核對欄位
...
def _normalize_inbody_visceral_fat(vf) -> float | None:
    """內臟脂肪級別（1~30），過濾刻度誤讀。"""
...
def _normalize_inbody_bmr_tdee(
    ...
    if b is not None and t is not None and t < b * 1.05:
        logger.warning("TDEE 不合理地接近或低於 BMR：bmr=%s tdee=%s", b, t)
        t = None

    w = _optional_positive_float(weight_kg)
    if w is not None:
        bmr_floor = max(1200.0, 12.0 * w)
        tdee_floor = max(2000.0, 18.0 * w)
        if b is not None and w >= 100 and b < bmr_floor:
            ...
            b = None
        if t is not None and w >= 100 and t < tdee_floor:
            logger.warning(
                "TDEE 與體重不匹配，棄用：tdee=%s weight=%s（預期至少約 %.0f）",
                t,
                w,
                tdee_floor,
            )
...
async def handle_inbody_photo(user_id: str, message_id: str) -> str:
    ...
    result = await call_openai_vision(
        ...
        user_prompt=(
            ...
            "BMR 在研究參數區、建議熱量攝取在該區最後一行；"
            "內臟脂肪讀「內臟脂肪級別」數字而非圖表刻度。"
        ),
        image_base64=image_b64,
        image_detail="high",
    )
    ...
    visceral_fat = _normalize_inbody_visceral_fat(result.get("visceral_fat"))

    bmr_f, tdee_f = _normalize_inbody_bmr_tdee(
        result.get("bmr"),
        result.get("tdee"),
        weight_kg=weight_f,
    )
    ...
    lines = [
        ...
        f"  內臟脂肪：{int(visceral_fat) if visceral_fat is not None else result.get('visceral_fat', '?')}",
...
def calculate_targets(weight, body_fat, bmr, tdee_ocr=None) -> dict:
    ...
    if t_ocr is not None:
        ...
        if bmr_ocr is not None and 900.0 <= bmr_ocr <= 3800.0:
            ref_bmr = bmr_ocr
        low = max(1100.0, ref_bmr * 1.12)
        high = min(7500.0, max(ref_bmr * 2.85, ref_bmr + 600.0))
        if t_ocr > ref_bmr * 1.05 and low <= t_ocr <= high:
            tdee = t_ocr
