...
- visceral_fat（內臟脂肪級別）— 極易讀錯，請嚴格遵守：
  - 只讀「內臟脂肪級別／Visceral Fat Level」【文字標籤同一行右側】的單一整數（例如 20）
  - 禁止讀取：長條圖左側刻度軸（1、5、10、15、20）、顏色區間標記、圖表網格數字
  - 若條形圖末端落在高風險區（橘/紅），級別常 ≥ 15，勿誤用刻度 10
...
def _resolve_inbody_tdee(
    ...
    """從 BMR、OCR TDEE 與研究參數區所有 kcal 候選值，解析可信 TDEE。"""
    ...
    min_ratio = 1.4
    if b is not None:
        valid = [c for c in candidates if c > b * min_ratio]
        if valid:
            best = max(valid)
...
async def handle_inbody_photo(user_id: str, message_id: str) -> str:
    ...
    result = await call_openai_vision(
        ...
        user_prompt=(
            ...
            "【TDEE】只填「建議的熱量攝取」標籤那一欄（研究參數區最後一行、通常是最大 kcal）；"
            "並列出 research_kcal_values 區塊內全部 kcal。\n"
            "【內臟脂肪】只讀「內臟脂肪級別」標籤同行的數字，勿讀長條圖刻度軸。"
        ),
        image_base64=image_b64,
        image_detail="high",
    )
