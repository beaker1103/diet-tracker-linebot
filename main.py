...
- 一旦你心中有任何「最低～最高」的熱量區間，回傳的 calories 必須取該區間的「上緣」（最大值）：例如區間約 250～350 kcal，則 calories 填 350（不可填中位或偏低值）
...
1. 先判斷食物種類與份量
2. 估算合理熱量區間（最低～最高）
3. 回報的 calories 一律取該區間上緣（最高值）；不要用中位數或 85～95 百分位折衷
...
- 一旦你心中有任何「最低～最高」的熱量區間，回傳的 calories 必須取該區間的「上緣」（最大值）：例如約 250～350 kcal，則 calories 填 350
...
請嚴格以下列 JSON 格式回覆，不要有其他文字：
{
  ...
  "estimation_note": "簡短說明為何給這個熱量數字（須反映已採區間上緣）"
}"""
...
【極重要：BMR 與 TDEE 不可混淆】
InBody／體組成報告上常同時出現多個「熱量」相關數字，語意不同：
- 「基礎代謝率」「BMR」「Basal Metabolic Rate」「基礎代謝」→ 填入 JSON 的 bmr（通常明顯低於每日總消耗）
- 「建議熱量攝取」「Recommended Calorie Intake」「建議卡路里」「一日能量」「TDEE」「總消耗」「Total Energy Expenditure」及與「活動量／運動」一併估算的每日熱量 → 填入 JSON 的 tdee（通常高於 BMR）
- 絕對禁止把「建議熱量攝取／TDEE」那一欄誤填進 bmr；bmr 與 tdee 必須分開填。若報告未列出某一項，該欄填 null。
- 合理檢查：同一張報告若兩者都有數字，應滿足 bmr < tdee（例外極少）；若不成立代表欄位對錯，請自行對調後再輸出。
...
def _normalize_inbody_bmr_tdee(bmr, tdee) -> tuple[float | None, float | None]:
    """修正模型偶發將 TDEE／BMR 對調的情形。"""
    b = _optional_positive_float(bmr)
    t = _optional_positive_float(tdee)
    if b is not None and t is not None and b >= t * 0.98:
        return t, b
    return b, t
...
async def handle_inbody_photo(user_id: str, message_id: str) -> str:
    ...
    bmr_f, tdee_f = _normalize_inbody_bmr_tdee(result.get("bmr"), result.get("tdee"))
    bmr = int(round(bmr_f)) if bmr_f is not None else None
    tdee = int(round(tdee_f)) if tdee_f is not None else None

    # 根據數據重新計算目標（優先採報告上的 TDEE）
    new_targets = calculate_targets(weight, bf, bmr, tdee)
...
def calculate_targets(weight, body_fat, bmr, tdee_ocr=None) -> dict:
    """
    每日熱量／蛋白質目標（減脂情境為主）。

    蛋白質：Helms et al. (2014) 熱量赤字下約 2.3–3.1 g/kg 去脂體重（LBM），
    並以 Morton et al. (2018) 約 1.62 g/kg 體重作為額外效益有限的上緣參考（取與 LBM 上限的較小值）。
    熱量：以 Katch–McArdle（有體脂時）估算 BMR；若 InBody 報告辨識出可信的 TDEE（建議熱量攝取），
    優先以其為 TDEE 計算赤字；否則 TDEE = BMR × PAL。赤字比例依體脂調整，並受絕對上限約束。
    """
    if not weight:
        return {"calories": DEFAULT_CALORIE_TARGET, "protein": DEFAULT_PROTEIN_TARGET}
    ...
    if t_ocr is not None:
        low = max(1100.0, bmr_used * 1.12)
        high = min(9000.0, max(bmr_used * 2.85, bmr_used + 600.0))
        if t_ocr > bmr_used * 1.05 and low <= t_ocr <= high:
            tdee = t_ocr
            use_device_tdee = True

    # 赤字：若採報告 TDEE，依體脂給較符合儀器情境的比例（高體脂約 33%）；否則沿用原 PAL 推算路徑
    if use_device_tdee:
        if bf is None:
            d_pct = 0.25
        elif bf >= 30:
    ...
    # 舊版絕對上限 1100 kcal 會卡住高 TDEE（例如報告 TDEE 要做約 33% 赤字）；裝置 TDEE 路徑改為僅受 TDEE 的 35% 上限約束
    deficit_cap = 0.35 * tdee if use_device_tdee else min(1100.0, 0.35 * tdee)
