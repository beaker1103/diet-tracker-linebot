...
- bmr：基礎代謝率（kcal）
  ...
  - 合理範圍約 1200~5000
- tdee：建議的熱量攝取（kcal）
  - 在右下角「建議的熱量攝取／Recommended Calorie Intake」
  - 通常高於 BMR，約為 BMR 的 1.5~2.0 倍
- body_water：體水分（kg）
- visceral_fat：內臟脂肪級別（整數）
...
3. bmr 與 tdee 皆存在時，tdee 必須 > bmr × 1.2；若不成立，優先將可疑值設 null
4. bmr 必須介於 1200~5000；tdee 必須介於 1400~9000
...
def _normalize_inbody_bmr_tdee(bmr, tdee) -> tuple[float | None, float | None]:
    """清理 BMR/TDEE，處理邊界值、對調與比例異常。"""
    b = _optional_positive_float(bmr)
    t = _optional_positive_float(tdee)

    # 先用硬邊界過濾明顯錯值
    if b is not None and not (1200.0 <= b <= 5000.0):
        b = None
    if t is not None and not (1400.0 <= t <= 9000.0):
        t = None

    if b is not None and t is not None:
        # TDEE 被塞到 bmr、bmr 被塞到 tdee 的常見情況
        if t < b:
            b, t = t, b

        ratio = t / b if b > 0 else None
        # 比例不合理時，優先嘗試對調；仍不合理則放棄可疑值
        if ratio is not None and (ratio < 1.2 or ratio > 2.5):
            swapped_ratio = (b / t) if t > 0 else None
            if swapped_ratio is not None and 1.2 <= swapped_ratio <= 2.5:
                b, t = t, b
            else:
                logger.warning(
                    "BMR/TDEE 比例異常，丟棄 TDEE：bmr=%s tdee=%s ratio=%.2f",
                    b,
                    t,
                    ratio,
                )
